...
func (self Instr) String() string {
	switch self.Op {
	case OP_ldconst:
		return fmt.Sprintf("ldconst     %s", self.Rv)
	case OP_ldvar:
		return fmt.Sprintf("ldvar       %s", self.Rv)
	case OP_store:
		return fmt.Sprintf("store       %s", self.Rv)
	case OP_define:
		return fmt.Sprintf("define      %s", self.Rv)
	case OP_car:
		return "car"
	case OP_cdr:
		return "cdr"
	...
	case OP_leave:
		return "leave"
	case OP_goto:
		return fmt.Sprintf("goto       @%d", self.Iv)
	case OP_if_false:
		return fmt.Sprintf("if_false   @%d", self.Iv)
	case OP_apply:
		return fmt.Sprintf("apply       %d", self.Iv)
	case OP_return:
		return "return"
	default:
...
func (self Compiler) compileList(p *Program, v *List) {
	...
	switch at {
	case "car":
		self.compileArgs(p, vv, 1)
		p.add(OP_car)
	case "cdr":
		self.compileArgs(p, vv, 1)
		p.add(OP_cdr)
	case "cons":
		self.compileArgs(p, vv, 2)
		p.add(OP_cons)
	case "eval":
		self.compileArgs(p, vv, 1)
		p.add(OP_eval)
	case "quote":
		self.compileQuote(p, vv)
	case "begin":
		self.compileBegin(p, vv)
	case Lambda:
		fallthrough
	case "lambda":
		self.compileLambda(p, vv)
	case "define":
		self.compileDefine(p, vv)
	case "if":
		self.compileCondition(p, v)
	case "do":
		self.compileList(p, self.desugarDo(vv))
	case "let":
		self.compileList(p, self.desugarLet(vv, true))
	case "let*":
		self.compileList(p, self.desugarLet(vv, false))
	default:
		p.i32(OP_apply, self.compileArgs(p, v, -1))
	}
}
...
func (self Compiler) desugarLet(v *List, isParallel bool) *List {
	...
	if decl, ok = AsList(p.Car); !ok {
		panic("compile: malformed let construct: " + v.String())
	}
	if p, ok = AsList(p.Cdr); !ok {
		panic("compile: malformed let construct: " + v.String())
	}
	if body, ok = AsList(p.Car); !ok {
		panic("compile: malformed let construct: " + v.String())
	}
	if p.Cdr != nil {
		panic("compile: malformed let construct: " + v.String())
